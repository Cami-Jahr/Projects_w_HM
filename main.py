...
width, height = 40, 40  # game.display_surf.get_size()
# image = game.getImage().transpose(2,1,0)
# plt.figure()
# plt.imshow(image)
# plt.show()
...
def select_action(state):
    ...
    eps_thr = EPS_END + (EPS_START - EPS_END) * math.exp(-1. * steps / EPS_DECAY)
    steps += 1
    if sample > eps_thr:
        with torch.no_grad():
...
for i_episode in range(num_episodes):
    ...
    last_screen = get_screen(game)  # get_screen()
    current_screen = get_screen(game)  # get_screen()
    state = current_screen - last_screen
    for t in count():
        # Select and perform an action
        action = select_action(state)
        done = game.execute(action.item())
